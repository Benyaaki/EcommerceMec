...
export function initSilk(canvas) {
  ...
  function animate() {
    ...
    // Gradiente base azul oscuro nórdico / noche profunda
    const gradient = ctx.createLinearGradient(0, 0, width, height);
    gradient.addColorStop(0, '#06090e');
    gradient.addColorStop(0.5, '#0d1522');
    ...
    for (let x = 0; x < width; x += step) {
      for (let y = 0; y < height; y += step) {
        ...
        // Ondas de seda azul pastel oscurecido
        const r = Math.floor(25 * intensity + 15);
        const g = Math.floor(48 * intensity + 28);
        const b = Math.floor(105 * intensity + 55);
        const a = Math.floor(220 * intensity);
    ...
    // Overlay viñeta radial oscura para contraste
    const overlayGradient = ctx.createRadialGradient(
      width / 2, height / 2, 0,
      width / 2, height / 2, Math.max(width, height) / 2
    );
    overlayGradient.addColorStop(0, 'rgba(13, 21, 34, 0.25)');
    overlayGradient.addColorStop(1, 'rgba(5, 7, 11, 0.85)');
